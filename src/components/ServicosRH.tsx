...
export const ServicosRH: React.FC<Props> = ({ onOpenNewChamado, refreshKey }) => {
  ...
  return <div className="space-y-4 sm:space-y-6 pb-8 sm:pb-12"><div className="bg-neutral-900 text-white p-4 sm:p-6 rounded-2xl sm:rounded-[2rem] flex flex-col sm:flex-row sm:items-center justify-between gap-4"><div><h1 className="text-xl sm:text-2xl font-black flex items-center gap-2"><LifeBuoy className="text-yellow-400 shrink-0"/>Meus Chamados</h1><p className="text-xs text-neutral-400 mt-2">Acompanhe os chamados que você abriu, o andamento e a resolução de cada solicitação.</p></div><button onClick={onOpenNewChamado} className="min-h-11 w-full sm:w-auto justify-center px-5 py-3 rounded-full bg-yellow-400 text-black font-black text-sm flex items-center gap-2"><Plus className="w-4 h-4"/>Abrir chamado</button></div>
