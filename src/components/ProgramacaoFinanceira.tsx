    ...
    return (
      <div className="space-y-6">
        ...
        {dre && <>
          ...
          <div className="rounded-3xl bg-neutral-50 border border-neutral-200 p-4 sm:p-6">
            <div className="mb-5 flex items-end justify-between gap-3"><div><h2 className="font-black text-neutral-900">Demonstrativo do resultado</h2><p className="text-xs text-neutral-500 mt-1">Arraste a visualização para o lado para percorrer as colunas da DRE.</p></div><span className="hidden sm:block text-[10px] font-bold text-neutral-400">← DESLIZE →</span></div>
            <div className="flex items-start gap-4 overflow-x-auto pb-4 snap-x snap-mandatory">
              {linhasDre.map((item) => {
                ...
                const estilo = DRE_GROUP_STYLE[item.codigoGrupo] || DRE_GROUP_STYLE['16'];
                return <div key={item.codigoGrupo} className={`w-[280px] sm:w-[300px] shrink-0 snap-start rounded-2xl bg-white border border-neutral-200 border-t-4 ${estilo.accent.replace('border-l-', 'border-t-')} p-4 shadow-sm`}><div className="flex items-start justify-between gap-3"><div className="flex items-center gap-3 min-w-0"><span className={`w-10 h-10 rounded-xl flex items-center justify-center text-xs font-black shrink-0 ${estilo.code}`}>{item.codigoGrupo}</span><h3 className="font-black text-neutral-900 leading-tight">{item.grupo}</h3></div><span className={`rounded-full px-2 py-1 text-[9px] font-black uppercase shrink-0 ${percentual >= 99.9 ? 'bg-emerald-100 text-emerald-700' : percentual > 0 ? 'bg-amber-100 text-amber-700' : 'bg-red-50 text-red-600'}`}>{percentual >= 99.9 ? 'Realizado' : percentual > 0 ? 'Parcial' : 'Pendente'}</span></div><div className="space-y-2.5 mt-5"><div className="rounded-xl bg-neutral-50 border border-neutral-100 p-3"><span className="text-[9px] font-bold uppercase text-neutral-400 block">Programado</span><strong className="font-mono text-base text-neutral-900">{formatCurrency(item.programado)}</strong></div><div className="rounded-xl bg-emerald-50 border border-emerald-100 p-3"><span className="text-[9px] font-bold uppercase text-emerald-600 block">Realizado</span><strong className="font-mono text-base text-emerald-700">{formatCurrency(item.realizado)}</strong></div><div className="rounded-xl bg-amber-50 border border-amber-100 p-3"><span className="text-[9px] font-bold uppercase text-amber-600 block">Diferença</span><strong className="font-mono text-base text-amber-800">{formatCurrency(diferenca)}</strong></div><div className="rounded-xl bg-neutral-900 p-3 text-white flex items-center justify-between"><span className="text-[9px] font-bold uppercase text-neutral-400">% realizado</span><strong className="text-base">{percentual.toFixed(1)}%</strong></div></div><div className="h-2 bg-neutral-100 rounded-full overflow-hidden mt-4"><div className={`h-full rounded-full ${estilo.bar}`} style={{ width: `${Math.min(percentual, 100)}%` }} /></div></div>;
              })}
              <div className="w-[280px] sm:w-[300px] shrink-0 snap-start rounded-2xl bg-gradient-to-b from-neutral-950 to-neutral-800 text-white p-5 shadow-lg border-t-4 border-yellow-400"><div className="flex items-center justify-between gap-3"><div><span className="text-[10px] text-yellow-400 font-black uppercase tracking-widest">Fechamento</span><h3 className="font-black text-lg mt-1">RESULTADO</h3></div><Target className="w-7 h-7 text-yellow-400" /></div><div className="space-y-3 mt-6"><div className="rounded-xl bg-white/5 p-3"><span className="text-[9px] uppercase text-neutral-400 block">Programado</span><strong className="font-mono">{formatCurrency(resultadoProgramado)}</strong></div><div className="rounded-xl bg-emerald-400/10 p-3"><span className="text-[9px] uppercase text-emerald-400 block">Realizado</span><strong className="font-mono text-emerald-400">{formatCurrency(resultadoRealizado)}</strong></div><div className="rounded-xl bg-white/5 p-3"><span className="text-[9px] uppercase text-neutral-400 block">Diferença</span><strong className="font-mono">{formatCurrency(resultadoProgramado - resultadoRealizado)}</strong></div></div></div>
            </div>
          </div>
