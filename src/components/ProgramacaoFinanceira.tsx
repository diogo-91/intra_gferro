      ...
      <div className="space-y-6">
        ...
        {dre && <>
          ...
          <div className="rounded-3xl bg-neutral-50 border border-neutral-200 p-4 sm:p-6">
            <div className="mb-5"><h2 className="font-black text-neutral-900">Demonstrativo do resultado</h2><p className="text-xs text-neutral-500 mt-1">Grupos apresentados verticalmente, seguindo a composição da DRE.</p></div>
            <div className="space-y-3">
              {linhasDre.map((item) => {
                ...
                return <div key={item.codigoGrupo} className={`rounded-2xl bg-white border border-neutral-200 border-l-4 ${estilo.accent} p-4 sm:p-5 shadow-sm`}><div className="flex items-center gap-3"><span className={`w-10 h-10 rounded-xl flex items-center justify-center text-xs font-black shrink-0 ${estilo.code}`}>{item.codigoGrupo}</span><div><h3 className="font-black text-neutral-900">{item.grupo}</h3><span className={`inline-flex mt-1 rounded-full px-2 py-0.5 text-[9px] font-black uppercase ${percentual >= 99.9 ? 'bg-emerald-100 text-emerald-700' : percentual > 0 ? 'bg-amber-100 text-amber-700' : 'bg-red-50 text-red-600'}`}>{percentual >= 99.9 ? 'Realizado' : percentual > 0 ? 'Parcial' : 'Pendente'}</span></div></div><div className="grid grid-cols-2 lg:grid-cols-4 gap-3 mt-4"><div className="rounded-xl bg-neutral-50 p-3"><span className="text-[9px] font-bold uppercase text-neutral-400 block">Programado</span><strong className="font-mono text-sm text-neutral-900">{formatCurrency(item.programado)}</strong></div><div className="rounded-xl bg-emerald-50 p-3"><span className="text-[9px] font-bold uppercase text-emerald-600 block">Realizado</span><strong className="font-mono text-sm text-emerald-700">{formatCurrency(item.realizado)}</strong></div><div className="rounded-xl bg-amber-50 p-3"><span className="text-[9px] font-bold uppercase text-amber-600 block">Diferença</span><strong className="font-mono text-sm text-amber-800">{formatCurrency(diferenca)}</strong></div><div className="rounded-xl bg-neutral-900 p-3 text-white"><span className="text-[9px] font-bold uppercase text-neutral-400 block">% realizado</span><strong className="text-sm">{percentual.toFixed(1)}%</strong></div></div><div className="h-1.5 bg-neutral-100 rounded-full overflow-hidden mt-4"><div className={`h-full rounded-full ${estilo.bar}`} style={{ width: `${Math.min(percentual, 100)}%` }} /></div></div>;
              })}
              <div className="rounded-2xl bg-gradient-to-r from-neutral-950 to-neutral-800 text-white p-5 shadow-lg"><div className="flex items-center justify-between gap-3"><div><span className="text-[10px] text-yellow-400 font-black uppercase tracking-widest">Resultado consolidado</span><h3 className="font-black text-lg mt-1">RESULTADO</h3></div><Target className="w-7 h-7 text-yellow-400" /></div><div className="grid sm:grid-cols-3 gap-3 mt-4"><div><span className="text-[9px] uppercase text-neutral-400 block">Programado</span><strong className="font-mono">{formatCurrency(resultadoProgramado)}</strong></div><div><span className="text-[9px] uppercase text-neutral-400 block">Realizado</span><strong className="font-mono text-emerald-400">{formatCurrency(resultadoRealizado)}</strong></div><div><span className="text-[9px] uppercase text-neutral-400 block">Diferença</span><strong className="font-mono">{formatCurrency(resultadoProgramado - resultadoRealizado)}</strong></div></div></div>
            </div>
          </div>
